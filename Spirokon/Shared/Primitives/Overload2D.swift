...
extension CGPoint: Overload2D {
    ...
    init(radius: CGFloat, theta: CGFloat = 0) {
        // Negate y so we end up with a normal Cartesian plane
        self.init(x: radius * cos(theta), y: -1.0 * radius * sin(theta))
    }

    static let one = CGPoint(x: 1, y: 1)
    static let unitCircle = CGPoint(radius: 1)
...
extension CGSize: Overload2D {
    ...
    static let unitCircle = CGSize(radius: 1)

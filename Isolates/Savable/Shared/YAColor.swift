...
extension YAColor {
    ...
    static func css(_ code: String) -> YAColor {
        ...
        var end = code.index(start, offsetBy: 2)
        let r = Double(Int(code[start..<end], radix: 16)!) / 256.0

        start = end
        end = code.index(start, offsetBy: 2)
        let g = Double(Int(code[start..<end], radix: 16)!) / 256.0

        start = end
        end = code.index(start, offsetBy: 2)
        let b = Double(Int(code[start..<end], radix: 16)!) / 256.0
